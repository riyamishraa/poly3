...
const BASE_PATH = "./circuits/riyaCustomCircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/riyaCustomCircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/riyaCustomCircuitVerifier.sol:Verifier");
